...
export default function Table({ tableHead, stats }) {
  ...
  return (
    <table className={style.statsTable}>
      <caption className={style.statsTable__title}>{stats[0].team.full_name}. Players statistics</caption>
      ...
      <tbody className={style.statsTable__type}>
        {stats.map(game => (
          ...
          <tr className={style.statsTable__row} key={game.id}>
            ...
            <td>{game.min.length > 1 && game.min[0] === '0' ? game.min.slice(1, 2) : game.min}</td>
